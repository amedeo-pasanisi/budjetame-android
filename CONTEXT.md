...
**Transfer**:
A Transaction that moves money from a Source Wallet to a Destination Wallet. Net Worth never changes, it never carries a Category, and the source and destination must be different Wallets — when the pair is exactly one own (non-Contact) Wallet and one Contact Wallet, it may also carry the matching-direction recurring link: from a Contact Wallet one links a Recurring Income, to a Contact Wallet one links a Recurring Cost (ADR-0027).
_Avoid_: internal transfer, move

**Recurring Cost**:
A definition of a cost expected to repeat at a fixed interval (every N days, weeks, months, or years), with a name and a fixed amount. It produces derived Occurrences; each payment is still recorded by hand as a linked Expense — or, when the payee is tracked as a Contact Wallet, as a linked Transfer to that Wallet (ADR-0027) — whose Wallet and Category are chosen at Transaction creation time; the definition itself never carries them. The app never creates Transactions on its own.
_Avoid_: monthly cost, fixed cost, subscription, recurring transaction

**Recurring Income**:
A definition of an income expected to repeat at a fixed interval (every N days, weeks, months, or years), with a name and a fixed amount. It produces derived Occurrences; each receipt is still recorded by hand as a linked Income — or, when the payer is tracked as a Contact Wallet, as a linked Transfer from that Wallet (ADR-0027) — whose Wallet and Category are chosen at Transaction creation time; the definition itself never carries them. The app never creates Transactions on its own.
_Avoid_: recurring earning, paycheck, salary entry

**Occurrence**:
One derived due instance of a Recurring Cost or Recurring Income, computed from the definition's start date plus k×interval (k = 0 is the start date itself). Its due date is its own date. Every definition has a start date: one left empty at creation is set to the creation day, so a fresh definition's first Occurrence is its creation day; afterwards the date can only be changed, never unset (ADR-0024). Each Occurrence is either Paid — exactly one linked Transaction covers it: an Expense or a Transfer to a Contact Wallet for a Cost (ADR-0027), an Income or a Transfer from a Contact Wallet for a Recurring Income (ADR-0027) — Unpaid, or Skipped: the user marked it as not applying, so it never enters the Backlog, never counts toward Monthly Spendable, and a link can never cover it. Un-skipping restores it to Unpaid.
...
- An Expense links to at most one Recurring Cost; linking pays exactly one Occurrence, the oldest Unpaid one — never a Skipped one; un-skipping comes first — pinned at link time and never reassigned by later date edits. Unlinking or deleting the Expense frees the Occurrence. An Income links to at most one Recurring Income under the same contract. A Transfer links under the same contract only when its legs are exactly one own Wallet and one Contact Wallet, and the direction picks the kind: from a Contact Wallet a Recurring Income (money in), to a Contact Wallet a Recurring Cost (money out). Own↔own and Contact↔Contact Transfers never qualify, and an edit that would leave a link on a no-longer-qualifying Transfer is rejected with a rule error naming the problem — never silently severed; unlinking always succeeds (ADR-0027).
- Recurring Cost names are unique per Account, case-insensitively; Recurring Income names are unique the same way.
- Deleting a Recurring Cost severs the links and drops its skips: linked Expenses — and linked Transfers to a Contact Wallet (ADR-0027) — remain as ordinary Transactions. Deleting a Recurring Income severs the links and drops its skips: linked Incomes — and linked Transfers from a Contact Wallet (ADR-0027) — remain as ordinary Transactions.
