 ...
 * the bucket is negative, the frame line "€Y this month (€X per day)"
 * (Monthly Spendable, then Daily Allowance), the bucket's transient
 * "€X over today's budget" note, and the Remaining Monthly Spendable bottom
 * line — "€X left this month", or the month's red "€X over this month's
 * budget" when the whole frame is blown. A negative Remaining Monthly
 * Spendable supersedes the bucket's note: never two over-notes at once. The
 * client only renders the endpoint's values; it never computes the frame
 * (ADR-0001).
 */
...
data class BudgetCardText(
    ...
    /** The frame line: `€Y this month (€X per day)`. */
